...
#else
    static SQClass * Create(SQClass *base) {
        SQClass *newclass = (SQClass *)SQ_MALLOC(sizeof(SQClass));
        new (newclass) SQClass(base);
        return newclass;
    }
#endif
    ~SQClass() ;
...
#else
    bool NewSlot(const SQObjectPtr &key,const SQObjectPtr &val,bool bstatic) ;
#endif
...
#else
    void Init() ;
    SQInstance(SQClass *c, SQInteger memsize) ;
    SQInstance(SQInstance *c, SQInteger memsize) ;
public:
    static SQInstance * Create(SQClass *theclass) {

        SQInteger size = calcinstancesize(theclass);
        SQInstance *newinst = (SQInstance *)SQ_MALLOC(size);
        new (newinst) SQInstance(theclass,size);
        if(theclass->_udsize) {
            newinst->_userpointer = ((unsigned char *)newinst) + (size - theclass->_udsize);
        }
        return newinst;
    ...
    SQInstance  *Clone()
    {
        SQInteger size = calcinstancesize(_class);
        SQInstance *newinst = (SQInstance *)SQ_MALLOC(size);
        new (newinst) SQInstance(this,size);
        if(_class->_udsize) {
            newinst->_userpointer = ((unsigned char *)newinst) + (size - _class->_udsize);
        }
        return newinst;
