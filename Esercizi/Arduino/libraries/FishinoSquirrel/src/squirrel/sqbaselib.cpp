...
static SQInteger  __getcallstackinfos(HSQUIRRELVM v,SQInteger level)
{
    ...
    if (SQ_SUCCEEDED(sq_stackinfos(v, level, &si)))
    {
        ...
        sq_newtable(v);
        sq_pushstring(v, _SC("func"), -1, SQTrue);
        sq_pushstring(v, fn, -1, SQFalse);
        sq_newslot(v, -3, SQFalse);
        sq_pushstring(v, _SC("src"), -1, SQTrue);
        sq_pushstring(v, src, -1, SQFalse);
        sq_newslot(v, -3, SQFalse);
        sq_pushstring(v, _SC("line"), -1, SQTrue);
        sq_pushinteger(v, si.line);
        sq_newslot(v, -3, SQFalse);
        sq_pushstring(v, _SC("locals"), -1, SQTrue);
        sq_newtable(v);
        seq=0;
        while ((name = sq_getlocal(v, level, seq))) {
            sq_pushstring(v, name, -1, SQFalse);
            sq_push(v, -2);
...
#else
        a = SQArray::Create(tointeger(size));
#endif
    }
    v->Push(a);
    return 1;
...
static SQInteger  base_type(HSQUIRRELVM v)
{
    SQObjectPtr &o = stack_get(v,2);
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    v->Push(SQString::Create(_ss(v),GetTypeName(o),-1, SQFalse));
#else
    v->Push(SQString::Create(GetTypeName(o),-1, SQFalse));
#endif
    return 1;
}
...
void  sq_base_register(HSQUIRRELVM v)
{
    ...
    while(base_funcs[i].name!=0) {
        sq_pushstring(v,base_funcs[i].name,-1, SQTrue);
        sq_newclosure(v,base_funcs[i].f,0);
        sq_setnativeclosurename(v,-1,base_funcs[i].name, SQTrue);
        sq_setparamscheck(v,base_funcs[i].nparamscheck,base_funcs[i].typemask);
        sq_newslot(v,-3, SQFalse);
        i++;
    }

    sq_pushstring(v,_SC("_versionnumber_"),-1, SQTrue);
    sq_pushinteger(v,SQUIRREL_VERSION_NUMBER);
    sq_newslot(v,-3, SQFalse);
    sq_pushstring(v,_SC("_version_"),-1, SQTrue);
    sq_pushstring(v,SQUIRREL_VERSION,-1, SQTrue);
    sq_newslot(v,-3, SQFalse);
    sq_pushstring(v,_SC("_charsize_"),-1, SQTrue);
    sq_pushinteger(v,sizeof(SQChar));
    sq_newslot(v,-3, SQFalse);
    sq_pushstring(v,_SC("_intsize_"),-1, SQTrue);
    sq_pushinteger(v,sizeof(SQInteger));
    sq_newslot(v,-3, SQFalse);
    sq_pushstring(v,_SC("_floatsize_"),-1, SQTrue);
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    v->Push(SQString::Create(_ss(v),(const SQChar *)&c, 1, SQFalse));
#else
    v->Push(SQString::Create((const SQChar *)&c, 1, SQFalse));
#endif
    return 1;
}
...
#else
    SQObjectPtr ret = SQArray::Create(size);
#endif
...
#else
    SQObjectPtr ret = SQArray::Create(0);
#endif
    SQInteger size = a->Size();
    SQObjectPtr val;
    for(SQInteger n = 0; n < size; n++) {
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    v->Push(SQString::Create(_ss(v),&_stringval(o)[sidx],eidx-sidx, SQFalse));
#else
    v->Push(SQString::Create(&_stringval(o)[sidx],eidx-sidx, SQFalse));
#endif
    return 1;
}
...
static SQInteger  string_find(HSQUIRRELVM v)
{
	...
	{\
	    ...
	    memcpy(snew,sthis,sq_rsl(len));\
	    for(SQInteger i=sidx;i<eidx;i++) snew[i] = func(sthis[i]); \
	    v->Push(SQString::Create(snew,len, SQFalse)); \
	    return 1; \
	}
#endif
...
static SQInteger  closure_getinfos(HSQUIRRELVM v) {
...
#else
        SQObjectPtr params = SQArray::Create(nparams);
        SQObjectPtr defparams = SQArray::Create(f->_ndefaultparams);
#endif
    ...
    for(SQInteger j = 0; j<f->_ndefaultparams; j++) {
        ...
        if(f->_varparams) {
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
            _array(params)->Set(nparams-1,SQString::Create(_ss(v),_SC("..."),-1, SQTrue));
        }
...
#else
            _array(params)->Set(nparams-1,SQString::Create(_SC("..."),-1, SQTrue));
        }
        res->NewSlot(SQString::Create(_SC("native"),-1, SQTrue),false);
        res->NewSlot(SQString::Create(_SC("name"),-1, SQTrue),f->_name);
        res->NewSlot(SQString::Create(_SC("src"),-1, SQTrue),f->_sourcename);
        res->NewSlot(SQString::Create(_SC("parameters"),-1, SQTrue),params);
        res->NewSlot(SQString::Create(_SC("varargs"),-1, SQTrue),f->_varparams);
        res->NewSlot(SQString::Create(_SC("defparams"),-1, SQTrue),defparams);
#endif
    }
...
#else
        res->NewSlot(SQString::Create(_SC("native"),-1, SQTrue),true);
        res->NewSlot(SQString::Create(_SC("name"),-1, SQTrue),nc->_name);
        res->NewSlot(SQString::Create(_SC("paramscheck"),-1, SQTrue),nc->_nparamscheck);
#endif
...
#else
                SQArray::Create(nc->_typecheck.size());
#endif
...
#else
        res->NewSlot(SQString::Create(_SC("typecheck"),-1, SQTrue),typecheck);
#endif
    }
    v->Push(res);
    return 1;
...
#else
        case SQGenerator::eSuspended:v->Push(SQString::Create(_SC("suspended"), -1, SQTrue));break;
        case SQGenerator::eRunning:v->Push(SQString::Create(_SC("running"), -1, SQTrue));break;
        case SQGenerator::eDead:v->Push(SQString::Create(_SC("dead"), -1, SQTrue));break;
#endif
    }
    return 1;
}
...
static SQInteger  thread_getstatus(HSQUIRRELVM v)
{
    ...
    switch(sq_getvmstate(_thread(o))) {
        case SQ_VMSTATE_IDLE:
            sq_pushstring(v,_SC("idle"),-1, SQTrue);
        break;
        case SQ_VMSTATE_RUNNING:
            sq_pushstring(v,_SC("running"),-1, SQTrue);
        break;
        case SQ_VMSTATE_SUSPENDED:
            sq_pushstring(v,_SC("suspended"),-1, SQTrue);
        break;
        default:
