...
SQSharedState::SQSharedState()
{
	...
	#define newsysstring(s) {   \
	    _systemstrings->push_back(SQString::Create(this,s, -1, SQTrue));    \
	    }
	
	#define newmetamethod(s) {  \
	    _metamethods->push_back(SQString::Create(this,s, -1, SQTrue));  \
	    _table(_metamethodsmap)->NewSlot(_metamethods->back(),(SQInteger)(_metamethods->size()-1)); \
	    }
#else
	#define newsysstring(s) {   \
	    _systemstrings->push_back(SQString::Create(s, -1, SQTrue));    \
	    }
	
	#define newmetamethod(s) {  \
	    _metamethods->push_back(SQString::Create(s, -1, SQTrue));  \
	    _table(_metamethodsmap)->NewSlot(_metamethods->back(),(SQInteger)(_metamethods->size()-1)); \
	    }
#endif
...
SQTable *CreateDefaultDelegate(SQSharedState *ss,const SQRegFunction *funcz)
{
    ...
    while(funcz[i].name!=0){
        ...
        nc->_name = SQString::Create(ss,funcz[i].name, -1, SQTrue);
        if(funcz[i].typemask && !CompileTypemask(nc->_typecheck,funcz[i].typemask))
            return NULL;
        t->NewSlot(SQString::Create(ss,funcz[i].name, -1, SQTrue),nc);
        i++;
    }
    return t;
}
#else
SQTable *CreateDefaultDelegate(const SQRegFunction *funcz)
{
    SQInteger i=0;
    SQTable *t=SQTable::Create(0);
    while(funcz[i].name!=0){
        SQNativeClosure *nc = SQNativeClosure::Create(funcz[i].f,0);
        nc->_nparamscheck = funcz[i].nparamscheck;
        nc->_name = SQString::Create(funcz[i].name, -1, SQTrue);
        if(funcz[i].typemask && !CompileTypemask(nc->_typecheck,funcz[i].typemask))
            return NULL;
        t->NewSlot(SQString::Create(funcz[i].name, -1, SQTrue),nc);
        i++;
    }
    return t;
}
...
void SQSharedState::Init()
{
...
#else
    new (_stringtable) SQStringTable();
#endif
...
#else
    _metamethodsmap = SQTable::Create(MT_LAST-1);
#endif
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    _constructoridx = SQString::Create(this,_SC("constructor"), -1, SQTrue);
...
#else
    _constructoridx = SQString::Create(_SC("constructor"), -1, SQTrue);
    _registry = SQTable::Create(0);
    _consts = SQTable::Create(0);
    ...
    _thread_default_delegate = CreateDefaultDelegate(_thread_default_delegate_funcz);
    _class_default_delegate = CreateDefaultDelegate(_class_default_delegate_funcz);
    _instance_default_delegate = CreateDefaultDelegate(_instance_default_delegate_funcz);
    _weakref_default_delegate = CreateDefaultDelegate(_weakref_default_delegate_funcz);
#endif
}
...
#ifndef SQUIRREL_SMALL_RESIZE_INCREMENT
            newsize = size + (size>>1);
#else
            newsize = size;
#endif
...
SQWeakRef *SQWeakRefTable::GetRef(SQObjectType type, SQRefCounted *refCounted)
{
	// look first into table
	for(SQUnsignedInteger i = 0; i < _links.size(); i++)
		if(_links[i].dest == refCounted)
			return _links[i].ref;
		
	// not found, append it
	SQWeakRefLink & link = _links.push_back();
	link.dest = refCounted;
	sq_new(link.ref, SQWeakRef);
#if defined(SQUSEDOUBLE) && !defined(_SQ64)
    link.ref->_obj._unVal.raw = 0; //clean the whole union on 32 bits with double
#endif
    link.ref->_obj._type = type;
    link.ref->_obj._unVal.pRefCounted = refCounted;
    return link.ref;
}

// remove weak ref for object
void SQWeakRefTable::RemoveRef(SQRefCounted *refCounted)
{
	for(SQUnsignedInteger i = 0; i < _links.size(); i++)
		if(_links[i].dest == refCounted)
		{
...
#ifdef UPP_TEST
/*
RLOG("Table using " << _numofslots << " slots, size " <<  _numofslots * sizeof(SQString*) << " bytes");
RLOG("Total size (besides string chars is " << _numofslots * (sizeof(SQString*) + sizeof(SQString)));
*/
#endif
...
SQSharedState *GlobalSharedstate = NULL;
#endif
