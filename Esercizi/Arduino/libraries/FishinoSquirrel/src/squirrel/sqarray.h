...
#else
    SQArray(SQInteger nsize){_values.resize(nsize); INIT_CHAIN();ADD_TO_CHAIN(&_ss(this)->_gc_chain,this);}
#endif
     ~SQArray()
    {
...
#else
    static SQArray * Create(SQInteger nInitialSize){
        SQArray *newarray=(SQArray*)SQ_MALLOC(sizeof(SQArray));
...
#else
    SQArray  *Clone(){SQArray *anew=Create(0); anew->_values.copy(_values); return anew; }
#endif
