...
public:
     SQCompiler(SQVM *v, SQLEXREADFUNC rg, SQUserPointer up, const SQChar* sourcename, bool raiseerror, bool lineinfo)
    {
        ...
        _sourcename = SQString::Create(_ss(v), sourcename, -1, SQFalse);
#else
        _lex.Init(rg, up,ThrowError,this);
        _sourcename = SQString::Create(sourcename, -1, SQFalse);
#endif
    ...
    SQObject  Expect(SQInteger tok)
    {
        ...
        {
        case TK_IDENTIFIER:
            ret = _fs->CreateString(_lex._svalue, -1, SQFalse);
            break;
        case TK_STRING_LITERAL:
            ret = _fs->CreateString(_lex._svalue,_lex._longstr.size()-1, SQFalse);
            break;
        case TK_INTEGER:
    ...
    bool  Compile(SQObjectPtr &o)
    {
        ...
        funcstate._name = SQString::Create(_ss(_vm), _SC("main"), -1, SQTrue);
#else
        SQFuncState funcstate(NULL,ThrowError,this);
        funcstate._name = SQString::Create(_SC("main"), -1, SQTrue);
#endif
        _fs = &funcstate;
        _fs->AddParameter(_fs->CreateString(_SC("this"), -1, SQTrue));
        _fs->AddParameter(_fs->CreateString(_SC("vargv"), -1, SQTrue));
        _fs->_varparams = true;
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
            _vm->_lasterror = SQString::Create(_ss(_vm), _compilererror, -1, SQFalse);
#else
            _vm->_lasterror = SQString::Create(_compilererror, -1, SQFalse);
#endif
            return false;
        }
        return true;
    ...
    SQInteger  Factor()
    {
        ...
        {
        case TK_STRING_LITERAL:
            _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString(_lex._svalue,_lex._longstr.size()-1, SQFalse)));
            Lex();
            break;
        case TK_BASE:
            Lex();
        ...
        case TK_THIS:{
                ...
                switch(_token) {
                    case TK_IDENTIFIER:  id = _fs->CreateString(_lex._svalue, -1, SQFalse);       break;
                    case TK_THIS:        id = _fs->CreateString(_SC("this"),4, SQTrue);        break;
                    case TK_CONSTRUCTOR: id = _fs->CreateString(_SC("constructor"),11, SQTrue); break;
                }
...
#pragma GCC diagnostic pop
    ...
    void  ParseTableOrClass(SQInteger separator,SQInteger terminator)
    {
        ...
        while(_token != terminator) {
            ...
            case TK_CONSTRUCTOR:{
                SQInteger tk = _token;
                Lex();
                SQObject id = tk == TK_FUNCTION ? Expect(TK_IDENTIFIER) : _fs->CreateString(_SC("constructor"), -1, SQTrue);
                Expect(_SC('('));
                _fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
                CreateFunction(id);
    ...
    void  ForEachStatement()
    {
        ...
        if(_token == _SC(',')) {
            ...
            idxname = _fs->CreateString(_SC("@INDEX@"), -1, SQTrue);
        }
        Expect(TK_IN);
        ...
        SQInteger itrpos = _fs->PushLocalVariable(_fs->CreateString(_SC("@ITERATOR@"), -1, SQTrue)); //use invalid id to make it inaccessible
    ...
    SQObject  ExpectScalar()
    {
        ...
        switch(_token) {
            ...
            case TK_FLOAT:
                ...
                break;
            case TK_STRING_LITERAL:
                val = _fs->CreateString(_lex._svalue,_lex._longstr.size()-1, SQFalse);
                break;
            case TK_TRUE:
            case TK_FALSE:
                val._type = OT_BOOL;
...
#else
        SQFuncState *funcstate = _fs->PushChildState();
#endif
        funcstate->_name = name;
        SQObject paramname;
        funcstate->AddParameter(_fs->CreateString(_SC("this"), -1, SQTrue));
        funcstate->_sourcename = _sourcename;
        SQInteger defparams = 0;
        while(_token!=_SC(')')) {
            if(_token == TK_VARPARAMS) {
                if(defparams > 0) Error(_SC("function with default parameters cannot have variable number of parameters"));
                funcstate->AddParameter(_fs->CreateString(_SC("vargv"), -1, SQTrue));
