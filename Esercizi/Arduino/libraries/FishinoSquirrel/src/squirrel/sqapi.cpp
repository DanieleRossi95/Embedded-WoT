...
HSQUIRRELVM sq_open(SQInteger initialstacksize)
{
	...
	{
	    sq_new(GlobalSharedstate, SQSharedState);
	    GlobalSharedstate->Init();
	}
#endif
    SQVM *v;
...
void sq_setbreakhook(HSQUIRRELVM vm, SQBREAKHOOK hook, SQInteger loops)
{
	_ss(vm)->_breakHook = hook;
	_ss(vm)->_breakLoops = loops;
	_ss(vm)->_breakCounter = loops;
}

void sq_clearbreakhook(HSQUIRRELVM vm)
{
	_ss(vm)->_breakHook = NULL;
	_ss(vm)->_breakLoops = 200;
	_ss(vm)->_breakCounter = 200;
}

SQUIRREL_API void sq_setshutdown(HSQUIRRELVM vm)
{
	_ss(vm)->_shutdown = SQTrue;
}

SQUIRREL_API void sq_clearshutdown(HSQUIRRELVM vm)
{
	_ss(vm)->_shutdown = SQFalse;
}

SQUIRREL_API SQBool sq_getshutdown(HSQUIRRELVM vm)
{
	return _ss(vm)->_shutdown;
}
...
#else
        v->Push(SQClosure::Create(_funcproto(o), _table(v->_roottable)->GetWeakRef(OT_TABLE)));
#endif
        return SQ_OK;
    }
    return SQ_ERROR;
...
void sq_pushstring(HSQUIRRELVM v,const SQChar *s,SQInteger len, SQBool isConst)
{
    if(s)
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
        v->Push(SQObjectPtr(SQString::Create(_ss(v), s, len, isConst)));
#else
        v->Push(SQObjectPtr(SQString::Create(s, len, isConst)));
#endif
    else v->PushNull();
}
...
#else
    SQUserData *ud = SQUserData::Create(size + SQ_ALIGNMENT);
#endif
    v->Push(ud);
...
#else
    v->Push(SQTable::Create(initialcapacity));
#endif
}
...
#else
    SQClass *newclass = SQClass::Create(baseclass);
#endif
    if(baseclass) v->Pop();
    v->Push(newclass);
    return SQ_OK;
...
void sq_newclosure(HSQUIRRELVM v,SQFUNCTION func,SQUnsignedInteger nfreevars, SQUnsignedInteger helper)
{
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    SQNativeClosure *nc = SQNativeClosure::Create(_ss(v), func,nfreevars, helper);
#else
    SQNativeClosure *nc = SQNativeClosure::Create(func,nfreevars, helper);
#endif
    nc->_nparamscheck = 0;
...
SQRESULT sq_setnativeclosurename(HSQUIRRELVM v,SQInteger idx,const SQChar *name, SQBool isConst)
{
    SQObject o = stack_get(v, idx);
...
#else
        nc->_name = SQString::Create(name, -1, isConst);
#endif
        return SQ_OK;
    }
...
#else
        _class(self)->NewSlot(key, v->GetUp(-1),false);
#endif
        v->Pop(2);
        return SQ_OK;
    break;
...
{
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    v->_lasterror=SQString::Create(_ss(v),err, -1, SQFalse);
#else
    v->_lasterror=SQString::Create(err, -1, SQFalse);
#endif
    return SQ_ERROR;
}
