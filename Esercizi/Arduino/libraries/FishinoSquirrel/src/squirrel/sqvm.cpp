...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    res = SQString::Create(_ss(this),_spval, -1, SQFalse);
#else
    res = SQString::Create(_spval, -1, SQFalse);
#endif
    return true;
}
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    dest = SQString::Create(_ss(this), _spval, l + ol, SQFalse);
#else
    dest = SQString::Create(_spval, l + ol, SQFalse);
#endif
    return true;
}
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    dest = SQString::Create(_ss(this),GetTypeName(obj1), -1, SQFalse);
#else
    dest = SQString::Create(GetTypeName(obj1), -1, SQFalse);
#endif
    return true;
}
...
#else
        SQArray *arr = SQArray::Create(nvargs);
#endif
...
#else
        SQGenerator *gen = SQGenerator::Create(closure);
#endif
...
#else
    SQClosure *closure = SQClosure::Create(func,_table(_roottable)->GetWeakRef(OT_TABLE));
#endif
...
exception_restore:
    //
    {
        ...
        {
            // shutdown check
            if(_ss(this)->_shutdown)
            {
                return false;
            }
            
            // break check
            if(_ss(this)->_breakHook && !_ss(this)->_breakCounter--)
            {
                // counter expired, call break hook
                _ss(this)->_breakHook(this);
                
                // reset break loops counter
                _ss(this)->_breakCounter = _ss(this)->_breakLoops;
...
#else
                    case NOT_TABLE: TARGET = SQTable::Create( arg1); continue;
                    case NOT_ARRAY: TARGET = SQArray::Create(0); _array(TARGET)->Reserve(arg1); continue;
#endif
...
bool SQVM::CallNative(SQNativeClosure *nclosure, SQInteger nargs, SQInteger newbase, SQObjectPtr &retval, bool &suspend)
{
    ...
    _nnativecalls++;
    SQInteger ret = (nclosure->_funchelped)(this, nclosure->_funchelper);
    _nnativecalls--;
...
#else
        if(!_class(self)->NewSlot(key,val,bstatic)) {
#endif
