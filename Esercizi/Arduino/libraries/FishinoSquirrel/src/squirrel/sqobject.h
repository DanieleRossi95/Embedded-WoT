...
struct SQRefCounted
{
		...
		SQBool _weakref : 1;
	#ifdef SQUIRREL_REFCOUNT_SIZE
		SQUnsignedInteger _uiRef : SQUIRREL_REFCOUNT_SIZE - 1;
	#else
		SQUnsignedInteger _uiRef : (sizeof(SQUnsignedInteger) - 1);
	#endif
	    SQRefCounted() { _uiRef = 0; _weakref = SQFalse; }

#else

	    struct SQWeakRef *_weakref;
	#ifdef SQUIRREL_REFCOUNT_SIZE
	    SQUnsignedInteger _uiRef : SQUIRREL_REFCOUNT_SIZE;
	#else
