...
#else
SQFuncState::SQFuncState(SQFuncState *parent,CompilerErrorFunc efunc,void *ed)
#endif
{
        _nliterals = 0;
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
        ...
        _sharedstate = ss;
#else
        _literals = SQTable::Create(0);
        _strings =  SQTable::Create(0);
#endif
        _lastline = 0;
        _optimization = true;
        _parent = parent;
...
{
    SQObjectPtr val;
    if(_table(SHAREDSTATE->_consts)->Get(name,val)) {
        e = val;
        return true;
    }
    return false;
...
SQObject SQFuncState::CreateString(const SQChar *s,SQInteger len, SQBool isConst)
{
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    SQObjectPtr ns(SQString::Create(_sharedstate,s,len, isConst));
#else
    SQObjectPtr ns(SQString::Create(s,len, isConst));
#endif
...
#else
    SQObjectPtr nt(SQTable::Create(0));
#endif
...
#else
SQFuncState *SQFuncState::PushChildState()
#endif
{
...
#else
    new (child) SQFuncState(this,_errfunc,_errtarget);
#endif
