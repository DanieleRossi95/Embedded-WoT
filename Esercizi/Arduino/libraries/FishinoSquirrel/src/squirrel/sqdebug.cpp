...
void  SQVM::Raise_Error(const SQChar *s, ...)
{
    ...
    _lasterror = SQString::Create(_ss(this),_spval,-1, SQFalse);
#else
    _lasterror = SQString::Create(_spval,-1, SQFalse);
#endif
}
...
SQString  *SQVM::PrintObjVal(const SQObjectPtr &o)
{
...
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
        return SQString::Create(_ss(this), _spval, -1, SQFalse);
#else
        return SQString::Create(_spval, -1, SQFalse);
#endif
        break;
    case OT_FLOAT:
    ...
    default:
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
        return SQString::Create(_ss(this), GetTypeName(o), -1, SQFalse);
#else
        return SQString::Create(GetTypeName(o), -1, SQFalse);
#endif
    }
}
...
{
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
    SQObjectPtr exptypes = SQString::Create(_ss(this), _SC(""), -1, SQTrue);
#else
    SQObjectPtr exptypes = SQString::Create(_SC(""), -1, SQTrue);
#endif
    SQInteger found = 0;
    for(SQInteger i=0; i<16; i++)
    {
        ...
        if(typemask & (mask)) {
#ifndef SQUIRREL_SINGLE_SHAREDSTATE
            if(found>0) StringCat(exptypes,SQString::Create(_ss(this), _SC("|"), -1, SQTrue), exptypes);
            found ++;
            StringCat(exptypes,SQString::Create(_ss(this), IdType2Name((SQObjectType)mask), -1, SQFalse), exptypes);
#else
            if(found>0) StringCat(exptypes,SQString::Create(_SC("|"), -1, SQTrue), exptypes);
            found ++;
            StringCat(exptypes,SQString::Create(IdType2Name((SQObjectType)mask), -1, SQFalse), exptypes);
#endif
        }
    }
