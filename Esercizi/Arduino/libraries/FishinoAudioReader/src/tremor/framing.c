...
int tremor_ogg_page_version(ogg_page *og)
{
	oggbyte_buffer ob;
...
int tremor_ogg_page_continued(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return -1;
...
int tremor_ogg_page_bos(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return -1;
...
int tremor_ogg_page_eos(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return -1;
...
ogg_int64_t tremor_ogg_page_granulepos(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return -1;
...
ogg_uint32_t tremor_ogg_page_serialno(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return 0xffffffffUL;
...
ogg_uint32_t tremor_ogg_page_pageno(ogg_page *og)
{
	oggbyte_buffer ob;
	if (oggbyte_init(&ob, og->header))
		return 0xffffffffUL;
...
int tremor_ogg_page_packets(ogg_page *og)
{
	int i;
	int n;
	int count = 0;
...
ogg_sync_state *tremor_ogg_sync_create(void)
{
...
int tremor_ogg_sync_destroy(ogg_sync_state *oy)
{
	if (oy)
	{
...
unsigned char *tremor_ogg_sync_bufferin(ogg_sync_state *oy, long bytes)
{
...
int tremor_ogg_sync_wrote(ogg_sync_state *oy, long bytes)
{
	if (!oy->fifo_head)
		return OGG_EINVAL;
...
long tremor_ogg_sync_pageseek(ogg_sync_state *oy, ogg_page *og)
{
	oggbyte_buffer page;
	long           bytes, ret = 0;
...
int tremor_ogg_sync_pageout(ogg_sync_state *oy, ogg_page *og)
{
...
int tremor_ogg_sync_reset(ogg_sync_state *oy)
{
...
ogg_stream_state *tremor_ogg_stream_create(int serialno)
{
...
int tremor_ogg_stream_destroy(ogg_stream_state *os)
{
	if (os)
	{
...
int tremor_ogg_stream_pagein(ogg_stream_state *os, ogg_page *og)
{
...
int tremor_ogg_stream_reset(ogg_stream_state *os)
{
...
int tremor_ogg_stream_reset_serialno(ogg_stream_state *os, int serialno)
{
...
int tremor_ogg_stream_packetout(ogg_stream_state *os, ogg_packet *op)
{
	return _packetout(os, op, 1);
}

int tremor_ogg_stream_packetpeek(ogg_stream_state *os, ogg_packet *op)
{
	return _packetout(os, op, 0);
}

int tremor_ogg_packet_release(ogg_packet *op)
{
	if (op)
	{
...
int tremor_ogg_page_release(ogg_page *og)
{
	if (og)
	{
...
void tremor_ogg_page_dup(ogg_page *dup, ogg_page *orig)
{
