...
\usepackage{caption}
%
\captionsetup[table]{name=Frammento}
%
...
\begin{center}
	...
	\vspace{3mm}
	{\LARGE{\bf di una piattaforma abilitante}}\\
	\vspace{3mm}
	{\LARGE{\bf del Web of Things}}\\
	\vspace{3mm}
	{\LARGE{\bf per dispositivi embedded}}\\
\end{center}
\vspace{25mm}
\par
\noindent
...
A me che non ho mai smesso di crederci,\\
Alla mia famiglia per avermi insegnato a non mollare mai,\\
A Vanessa che ha condiviso con me questa ulteriore esperienza,\\
A mia nonna che sarebbe stata fiera di me \ldots                      %\ldots lascia tre puntini
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%non numera l'ultima pagina sinistra
\clearpage{\pagestyle{empty}\cleardoublepage}
\renewcommand*{\listtablename}{Elenco dei frammenti di codice}
...
Il settore dell'IoT nasce nei primi anni dello scorso decennio. Inizialmente i suoi margini di applicazione erano pressoché limitati a causa della ridotta disponibilità di dispositivi compatibili con questo settore e a causa della complessa interoperabilità tra di essi. Il panorama è cambiato soltanto recentemente. Si è assistito negli ultimi anni ad una importante processo di espansione che, molto probabilmente, sarà destinato a durare per i prossimi anni avvenire. Se nel 2015, il numero di dispositivi connessi era di 15.41 miliardi, attualmente se ne contano circa 30, con una proiezione di 75.44 miliardi entro il 2025\cite{K1}.

Tra le principali critiche mosse al settore dell'Internet of Thing (IoT) possiamo sicuramente includere la difficile interoperabilità tra i numerosissimi prodotti esistenti che si basano spesso su protocolli e formati differenti e che non sono in grado di interagire tra di loro.

Al giorno d'oggi, il campo dell'IoT è riuscito a compiere passi molto importanti per far fronte ai vari limiti presenti all'interno dei suoi campi di applicazione. Uno di questi è sicuramente legato al problema della connettività. A questo proposito, osservando le importanti migliorie apportate dall'applicazione della tecnologia 5G in altri settori come ad esempio alla rete mobile, si è pensato di introdurre la suddetta tecnologia nel campo dell'IoT. Dalla sua applicazione si sono riscontrati miglioramenti riguardanti la fruizione di servizi a banda larga e a bassa latenza\cite{K2}.

Come precedentemente affermato, la connettività non è l'unico limite presente nel mondo dell'IoT. L'interoperabilità rappresenta sicuramente un ulteriore punto di riflessione. Grazie all'applicazione e allo sviluppo del paradigma del Web of Things (WoT), si è cercato di gestire questo importante limite. Nello specifico, è stato proprio grazie all'utilizzo di standard e tecnologie web già ampiamente consolidate, che è stato possibile affrontare il problema della difficile interoperabilità tra i prodotti esistenti all'interno del mondo dell'IoT. La standardizzazione promossa fin dal 2015 dal World Wide Consortium (W3C) ha cercato di ridurre la vasta frammentazione presente all'interno dell'IoT concentrandosi su componenti complementari che permettano di far integrare tra di loro differenti piattaforme e domini applicativi. Al sapiente lavoro di standardizzazione partecipano non solo i membri del W3C ma diversi collaboratori provenienti da importanti aziende tecnologiche a livello mondiale.

Lo scopo della seguente tesi è la progettazione e l'implementazione dello stack software WoT Servient per sistemi IoT, seguendo con precisione l'architettura proposta del W3C. La piattaforma si compone di un'interfaccia a linea di comando attraverso la quale l'utente è in grado di definire Thing Descriptions, le quali verranno usate come basi per la creazione di script eseguibili dai sistemi di questo tipo. I microcontrollori, in questo modo, assumeranno il ruolo di Thing ed esporranno le loro funzionalità agli utenti con cui interagiranno. A seguire viene descritta la struttura dell'elaborato.

Nella prima parte (capitoli 1 e 2) viene esposto lo stato dell'arte. Il capitolo 1 contiene una panoramica sull'IoT con cenni storici, la sua evoluzione, le sue caratteristiche e le sue criticità. Nel capitolo 2 si analizzerà il W3C WoT, illustrandone l'utilità e  l'architettura e mostrando i suoi casi d'uso, gli elementi principali e l'implementazione del Servient. 
...
\addtocontents{toc}{\string\let\string\cftchapfont\string\Large\bfseries}
\addtocontents{toc}{\string\let\string\cftchappagefont\string\Large\bfseries}
\addtocontents{toc}{\vspace{0.4cm}}
\addcontentsline{toc}{chapter}{I\HS\HS Stato dell'arte}
...
Il concetto dell'Internet of Things come lo conosciamo oggi è il risultato di un lunga fase di studio e di ricerca protrattasi per più di un decennio. 

La prima comparsa del termine risale al 1999 e la sua paternità è da attribuire a Kevin Ashton, cofondatore e direttore esecutivo di Auto-ID Center (consorzio di ricerca con sede al Massachusetts Institute of Technology), il quale, all'epoca, si riferiva solo ed esclusivamente a quei dispositivi connessi che potevano essere identificati in modo univoco tramite la tecnologia Radio-Frequency IDentification (RFID)\cite{K3}. 
...
Un requisito fondamentale del paradigma IoT è quello di rendere le Thing presenti in rete interconnesse tra loro, in grado di comunicare e scambiare informazioni. Il suo soddisfacimento, però, viene complicato dalla miriade di Thing presenti in commercio, ognuna con uno specifico hardware e uno specifico software che sono causa di frammentazione e di scarsa interoperabilità. Oltre a questo, si aggiunge il fatto che Thing diverse possono essere utilizzate per uno stesso scenario poiché, ad esempio, eseguono la stessa funzione. Quindi deve essere presente un sistema di regole che stabilisca in che modo le diverse tecnologie si relazionino le une con le altre e quale configurazione scegliere nel deployment dei differenti scenari. L'architettura dell'IoT si focalizza principalmente nella definizione delle regole sopra citate.

L'architettura dell'IoT è costituita da quattro livelli, ognuno dei quali specifica una diversa funzionalità\cite{K2}:

\begin{enumerate}[itemsep=0pt,partopsep=1pt,parsep=2pt]  
	\item \textbf{Livello di Sensing}\\
	Fanno parte di questo livello i sistemi hardware e i loro sensori. Questi ultimi sono in grado di collezionare dati provenienti dall'ambiente esterno, come ad esempio: temperatura, umidità, qualità dell'aria, velocità, rumore, pressione, movimento ecc. In alcuni casi possono avere una minima capacità di storage al fine di memorizzare un certo numero di misurazioni. Queste misurazioni, poi, sono convertite in segnali comprensibili alla Thing ad essi collegata.\\ 
...
Esistono diversi livelli di interoperabilità\cite{K5}, circa uno per ogni livello IoT precedentemente descritto. Ora di seguito li andremo ad esaminare.
...
L'importanza dell'interoperabilità nell'IoT è stata enfatizzata sia dall'accademia, sia dal settore industriale. La strada che si è scelto di percorrere è quella della standardizzazione, cioè la definizione di standard che rappresentino la base di riferimento da seguire per la realizzazione di tecnologie fra loro compatibili.
...
A grandi linee, il fine dell'architettura del WoT è quello di descrivere ciò che esiste piuttosto di dire cosa deve essere implementato, non viene definita nessuna tecnologia o implementazione concerta. Per questo motivo viene descritta un'architettura astratta, basata su un insieme di requisiti derivanti da vari casi d'uso (use-case), ognuno per un diverso dominio applicativo. Questi requisiti sono classificati in base all'ambito di applicazione in diverse "categorie" chiamate \textit{building blocks}, che possono essere viste come i pilastri che ogni piattaforma basata sul Web of Things deve rispettare (processo di standardizzazione). L'architettura del WoT li descrive uno per uno e definisce il modo in cui sono correlati. I building blocks, poi, possono essere mappati su una varietà di deployment concreti.

Le varie sezioni dell'architettura possono essere normative o informative e per ognuna di esse viene specificato a quale delle due categorie appartiene.
...
Una Thing dovrà essere attivata su un host connesso alla rete, in modo tale che potrà essere acceduta tramite un URI (Uniform Resource Identifier) ed internamente dovrà implementare uno stack software che, tramite l'interfaccia web, ne permetterà l'interazione. 
...
Di seguito si andranno ad analizzare i quattro Building Blocks, necessari al fine di implementare sistemi che risultino conformi all'architettura del WoT.
...
La WoT Thing Description\cite{K10} è un building block fondamentale e può essere considerata come l'entry point di una Thing (come lo è, ad esempio, la pagina \textit{index.html} di un sito Web). La TD definisce due componenti: un modello di informazione basato su un vocabolario semantico che deve essere rispettato dalla Thing corrispondente e, come si è detto precedentemente, una rappresentazione serializzata basata sul formato dei dati JSON. Sia il modello di informazione sia il formato di rappresentazione sono allineati con i Linked Data\cite{K11}, in questo modo le varie implementazioni possono scegliere di utilizzare il formato JSON-LD\cite{K12} ed i database a grafo per sfruttare al massimo l'elaborazione dei meta-dati. 

E' stato scelto il formato JSON per due motivi principali: la sua ampia diffusione nelle applicazioni Web, in modo da non incidere sulla curva di apprendimento dei sistemi WoT e la sua capacità di essere facilmente parsabile dai dispositivi informatici. Questa caratteristica lo rende particolarmente adatto per applicazioni che implementano comunicazioni machine-to-machine (M2M). Attualmente, il formato JSON-LD offre un buon compromesso tra semantica comprensibile alla macchina e usabilità per gli sviluppatori.
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=1pt]
	...
	\item \textbf{Azione}\\
	Un'azione consente di invocare una funzione sulla Thing. La sua invocazione può produrre come effetto un cambiamento di uno o più stati di una Thing.\\
	...
	Un esempio di un evento è il trigger di un allarme.
\end{itemize}
...
Le Thing, generalmente, memorizzano la propria TD in una directory che agisce da cache, velocizzandone così l'accesso e l'utilizzo da parte dei Consumers e facilitando la gestione dei sensori e attuatori connessi. 
...
In una piattaforma IoT può essere presente qualsiasi tipologia di dispositivi, ognuno con un differente set di componenti e di protocolli di comunicazione. Questo perché non esiste un singolo protocollo che vada bene per qualsiasi scenario ed è anche per questo motivo che il Web of Things si sta focalizzando sullo studio di come rendere dispositivi diversi interoperabili. 
...
Per ogni tipologia di piattaforma IoT, i Binding Templates illustrano quali siano i protocolli più adatti e per ciascun protocollo associano i meta-dati che saranno, successivamente, aggiunti alla TD. Ovviamente, si fa riferimento soltanto a quei protocolli che supportano le tecnologie del Web come ad esempio HTTP, COAP o MQTT. Nella scelta del protocollo di comunicazione si deve tener conto del tipo di architettura sottostante e di quali metodi vengono esposti. Esempi di pattern architetturali sono Restful e PubSub, mentre esempi di metodi ad essi collegati sono GET, PUT, POST, DELETE, PUBLISH e SUBSCRIBE. 

I Consumers che consumano una TD devono implementare il corrispondente
Protocol Binding per poter accedere alle funzionalità della Thing e invocarne le funzioni, andando ad includere lo stack tecnologico necessario e la sua configurazione in base alle informazioni trovate nella TD stessa.
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	...
	Si tratta dei protocolli standard dello strato Applicazione (HTTP, MQTT, CoAP, ecc.) senza alcun tipo di opzioni specifiche o meccanismi di sotto-protocollo.
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=1pt]
	...
	\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=1pt]
		...
		\item Informare gli utenti riguardo quali informazioni personali possono essere esposte e quali no
	\end{itemize}
...
Nella Figura 2.5 è mostrata una possibile configurazione di un'architettura del WoT, dove per ogni componente è specificata la struttura dei building blocks che lo compongono.
...
La comunicazione tra una ConsumedThing e un'ExposedThing implementate da due Servient può essere diretta o indiretta. \`E diretta nel caso in cui entrambi i Servient utilizzano gli stessi protocolli di rete, risultando così mutualmente accessibili, mentre è indiretta nel caso contrario, cioè quando i Servient utilizzano protocolli diversi o sono connessi a reti differenti che necessitano di un'autorizzazione per potervi accedere (es. firewall).  
...
Se nelle sezioni precedenti abbiamo descritto l'architettura astratta del WoT, ora, attraverso il Servient, ne consideriamo l'implementazione concreta.
\\
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	%   per here, cioè la figura va qui
	\begin{center}                          %centra nel mezzo della pagina
...
Nella Figura 2.6 sono mostrati i moduli che compongono l'architettura di un Servient. Di seguito verranno specificati, per ogni modulo, il suo ruolo e le sue funzionalità:
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	...
	La logica della Thing e il suo modello di interazione sono implementati in un sistema a runtime, chiamato WoT Runtime. Esso mantiene l'ambiente di esecuzione per l'implementazione del comportamento della Thing, è in grado sia di esporre e consumare Thing, sia di prelevare, processare, serializzare TD. Solitamente la logica dell'applicazione dovrebbe essere eseguita in un ambiente isolato in modo tale da prevenire accessi non autorizzati a dati sensibili (Private Security Data).\\
...
Nella sezione precedente sono stati descritti i componenti dell'architettura del Web of Things e l'implementazione astratta del WoT Servient. 
...
Attraverso questo tool l'Arduino viene convertito in un Web Server configurabile, il quale può interagire con uno o più sensori o attuatori. Le richieste sono gestite in un formato JSON non standard, in quanto si ha una diversa rappresentazione dei dati in memoria in modo da ridurre lo spazio da loro occupato. Infatti, gli sviluppatori hanno improntato tutto il progetto in tal senso, definendo dei meccanismi e delle strutture dati per diminuire sia la quantità di memoria occupata durante l'esecuzione del sistema sia la complessità computazionale, in modo da velocizzare le interazioni tra client e server e tra server ed i suoi sensori o attuatori.
...
Per generare gli eventi, invece, si deve chiamare il metodo\\\verb"void raise_event(Symbol event, ...)", il quale reindirizza la chiamata all'handler definito come segue: \verb"void (*EventFunc)(Symbol event, Thing *thing, ...)".

Per visualizzare la TD si utilizza il comando \verb"print()", mentre per invocare il garbage collector si usa il comando \verb"WebThings::collect_garbage()" fornito dalla libreria \textit{WebThings.h}. Questa libreria fornisce anche i metodi di gestione delle Interaction Affordances citati precedentemente.
...
\newgeometry{right=2.5cm}         
\chapter*{\vspace*{\fill} \begin{center} \centering{Parte 2\\ \vspace{15pt} Embedded WoT Servient} \end{center} \vspace{\fill}}
\thispagestyle{empty}
\restoregeometry

% comandi per modificare il font del capitolo e della pagina nella toc
\addtocontents{toc}{\string\let\string\cftchapfont\string\Large\bfseries}
\addtocontents{toc}{\string\let\string\cftchappagefont\string\Large\bfseries}
\addtocontents{toc}{\vspace{0.4cm}}
\addcontentsline{toc}{chapter}{\Large II\HS\HS Embedded WoT Servient}
\addtocontents{toc}{\vspace{0.2cm}}
\addtocontents{toc}{\string\let\string\cftchapfont\string\bfseries}
\addtocontents{toc}{\string\let\string\cftchappagefont\string\bfseries}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Lo scopo del progetto è lo studio e la realizzazione dello stack software WoT Servient per sistemi embedded (es. Arduino, NodeMCU ecc.), rispettando le specifiche ed i componenti della sua architettura, descritta nel \hyperref[chap:WoT]{capitolo 2}.
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	\item \textbf{Creazione Thing Description}\\
	...
	\item \textbf{Esposizione Thing Description}\\
	La Thing deve essere processata e renderizzata leggendo la sua TD.
	L'utente deve poter accedere e interagire con le sue Interaction Affordances, per cui, in dettaglio, deve poter leggere il contenuto delle sua proprietà, invocare le azioni e ricevere gli eventi in tempo reale.
	\item \textbf{Creazione file di scripting}\\
	L'utente, una volta definita la TD, deve poter creare il file di scripting (sketch) nel linguaggio Embedded-C compatibile con i sistemi embedded. Lo sketch contiene la logica dell'applicazione e viene eseguito dalla Thing.
	\item \textbf{Compilazione file di scripting}\\
	L'utente, una volta creato il file di scripting, deve poterlo compilare sulla specifica board scelta come Thing, così da verificare la presenza di eventuali errori sintattici e/o semantici da correggere.
	\item \textbf{Flashing file di scripting}\\
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	\item \textbf{Visualizzazione Thing Description}\\
	...
	\item \textbf{Protocol Bindings}\\
	Dato che le interazioni con la Thing devono avvenire tramite le tecnologie del Web, è necessario fornire un binding con i protocolli compatibili con questo tipo di tecnologie, che permettono lo scambio di messaggi tra entità connesse alla rete Internet, come ad esempio l'HTTP e le WebSocket.
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=13cm]{A.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Architettura dell'Embedded WoT Servient]{Architettura dell'Embedded WoT Servient}
	\end{center}
\end{figure}

\subsection{Componenti}

L'unico componente del sistema con cui l'utente interagisce è la \textbf{Thing CLI} (command-line interface). La Thing CLI è un'interfaccia a riga di comando che, tramite una procedura guidata completamente interattiva, permette all'utente di definire la logica della Thing, in particolare di:

\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=1pt]
	\item inserire i meta-dati della Thing Description e salvarla in un file JSON
	\item inserire le proprietà, specificando per ciascuna il nome ed il tipo
	\item inserire le azioni, specificando per ognuna il nome, l'insieme dei parametri in input (nome e tipo), il tipo di output ed il corpo della funzione invocata 
	\item inserire gli eventi, specificandone il nome ed opzionalmente gli schemi di sottoscrizione, notifica e cancellazione a cui i relativi messaggi devono sottostare
	\item inserire l'SSID e la password della rete a cui la Thing si dovrà connettere
	\item inserire le porte per l'accesso ad Internet e al canale WebSocket
	\item inserire librerie, costanti, variabili e funzioni addizionali e codice generico al file di scripting, sia come integrazione delle funzionalità già offerte dalla Thing, sia per aggiungerne di nuove
	\item inserire il corpo delle funzioni sia tramite riga di comando sia da file
	\item inserire nuovi tipi di messaggi da poter essere scambiati lungo il canale WebSocket
	\item compilare il file di scripting sulla board 
	\item caricare il file di scripting compilato sulla board per la sua esecuzione
\end{itemize}
...
Le principali librerie di Python che ho utilizzato insieme a \verb"click" e \verb"jinja2" sono:
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	...
	\item \textbf{sys}\\
	Questo modulo fornisce l'accesso sia a variabili, sia a funzioni che interagiscono con l'interprete di Python. E' stato impiegato per la gestione della terminazione del programma principale a seguito di errori nell'inserimento dei dati. Gli errori possono riguardare ad esempio il fallimento della validazione della TD o la mancata terminazione con successo dei processi di compilazione e caricamento dello sketch.
	\item \textbf{json e jsonschema}\\
	Della libreria \verb"json" è stato utilizzato il metodo \verb"load" per convertire un file o una stringa in formato json in un dictionary e il metodo \verb"dump" per l'operazione opposta. La libreria \verb"jsonschema", invece, è servita per validare la Thing Description specificata dall'utente rispetto allo schema JSON stabilito dal W3C. L'utente, infatti, può proseguire nella definizione della Thing solamente nel caso in cui la TD sia valida.
	\item \textbf{subprocess e shlex}\\
	Il modulo \verb"subprocess" è stato utilizzato per eseguire comandi bash tramite Python, mentre il modulo \verb"shlex" è stato impiegato per parsare correttamente i comandi bash e renderli eseguibili dalle shell UNIX. Si è usufruito dei due moduli per eseguire i comandi dell'interfaccia arduino-cli in modo trasparente per l'utente.
\end{itemize}

\subsection{Click}

La libreria Click permette di creare interfacce a linea di comando, altamente configurabili, con l'utilizzo di relativamente poche istruzioni. I principali punti di forza sono: la possibilità di definire comandi innestati, la generazione automatica della pagina di help e il supporto del caricamento lazy dei sotto-comandi a runtime. \`E una libreria strutturata a classi, in cui si possono definire tipi e comandi personalizzati, ma anche decoratori per i comandi stessi.
I metodi che mette a disposizione sono semplici e molto intuitivi, eccone alcuni che ho utilizzato per la realizzazione della Thing CLI:

\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=1pt]
	\item \verb"@click.group()": consente la definizione di un gruppo di comandi
	\item \verb"@click.command()": permette di definire uno specifico comando
	\item \verb"click.invoke(nome_comando)": consente di invocare il comando passato tra parentesi
	\item \verb"@click.pass_context": passa il contesto da un comando ad un altro all'interno di un gruppo di comandi
	\item \verb"click.prompt()": permette l'inserimento di dati (input) da tastiera  
	\item \verb"click.confirm()": rappresenta una scelta per l'utente, il quale la può confermare o rifiutare
	\item \verb"click.echo(stringa)": stampa a schermo la stringa in input (funziona come una semplice \verb"print()" di Python)
\end{itemize}

I metodi preceduti dalla \verb"@" rappresentano delle macro. Ad ogni comando deve essere associata una funzione chiamata callback, la quale viene invocata nel momento in cui si digita il comando corrispondente sul terminale e si clicca INVIO. Click permette anche di chiamare un comando all'interno di un altro inserendo nel codice del chiamante il metodo \verb"invoke()" e passandogli il comando chiamato. \`E possibile, infine, associare delle opzioni e argomenti ad un determinato comando, rispettivamente, attraverso i metodi \verb"click.option()" e \verb"click.argument()", ma non li ho utilizzati nell'implementazione del Servient.

Per creare una CLI innanzitutto occorre definire la callback generale che incorpora i vari comandi e solo successivamente le callback di ogni comando. Grazie a questa gerarchia, all'interno del metodo \verb"main" dell'applicazione è necessario invocare solamente la callback generale, a cui poi spetterà il compito di chiamare gli altri comandi.

\subsection{Jinja2}

Jinja2 è un template engine compatibile con il linguaggio Python, di cui condivide la sintassi di alcuni costrutti. Viene utilizzato principalmente da framework per applicazioni Web come Flask\footnote{\url{https://palletsprojects.com/p/flask/}}. Supporta l'ereditarietà dei template, in quanto è possibile usare uno stesso layout o uno simile per tutti i template del sistema, possiede una sandbox e supporta completamente la codifica Unicode. Nonostante venga impiegato maggiormente per creare file in HTML, XML o in altri formati di markup, Jinja2 può essere utilizzato per creare qualsiasi tipo di file dato che il file in output non necessita di un'estensione specifica.

Un file di template è costituito da una parte statica e da una parte dinamica. La prima rimane sempre fissa, non viene modificata nelle diverse esecuzioni del template, mentre la seconda, come lo si evince dal nome, cambia a seconda del tipo di istanza con cui si fa la resa del template. Solitamente, nella parte dinamica sono presenti variabili e/o espressioni che vengono rimpiazzate con dei valori specifici nel momento della resa del template. La componente dinamica di Jinja2 è rappresentata da una variabile racchiusa tra parentesi graffe doppie, in questo modo: \verb"{{ nome_variabile }}".

L'interazione con Jinja2 richiede tre fasi principali:

\begin{enumerate}[itemsep=0pt,partopsep=1pt,parsep=3pt]
	\item Nella prima fase si deve caricare il file di template all'interno dell'ambiente di esecuzione tramite la classe \verb"loader" e poi successivamente lo si richiama tramite il metodo \verb"get_template()".
	\item Nella seconda fase si istanziano le variabili di template con i valori corrispondenti.
	\item Nella terza fase, infine, si fa la resa del template tramite il metodo \verb"render()", a cui si devono passare in input le variabili istanziate nella fase precedente. Si devono passare tutte le variabili di cui il template fa uso come in un dizionario: le chiavi sono i nomi delle variabili, mentre i valori possono essere di qualsiasi tipo ma solitamente sono oggetti poiché hanno una capacità espressiva più elevata dei tipi base e consentono di ridurre il numero di variabili da dover passare. Il risultato della terza fase è la stringa che si ottiene dalla sostituzione delle variabili con i valori effettivi, la quale può essere salvata in un file. Questo processo prendere il nome di rendering.
\end{enumerate}

In Jinja2, infine, sono presenti alcuni dei costrutti tipici dei linguaggi di programmazione, come ad esempio l'\verb"if", il ciclo \verb"for" e le funzioni che prendono il nome di macro.

\subsection{Arduino-cli}

Arduino-cli, come ho specificato nel \hyperref[chap:Prog]{capitolo 3}, è un'interfaccia a riga di comando sviluppata da Arduino per i sistemi embedded. E' un container di comandi e per ognuno di loro è dedicata una pagina di documentazione, la quale può essere visionata tramite il comando \verb"help". Supporta la maggior parte delle tipologie di microcontrollori e delle librerie a loro dedicate. E' la controparte a riga di comando dell'IDE di Arduino,
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=11cm]{AC.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Interfaccia di arduino-cli]{Interfaccia di arduino-cli}
	\end{center}
\end{figure}

I comandi principali che mette a disposizione dell'utente sono:

\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=3pt]
	\item \verb"init": inizializza l'ambiente di Arduino e genera il file di configurazione \textit{arduino-cli.yaml}
	\item \verb"compile": effettua la compilazione dello sketch nella board specificata. 
	\item \verb"upload": effettua il flashing dello sketch nella board connessa alla porta specificata.
	\item \verb"core": permette di visualizzare i core (driver) per i microcontrollori installati e di installarne di nuovi. Questo comando svolge una funzione molto importante, infatti è possibile compilare e caricare uno sketch in uno specifico sistema solo se si posseggono i relativi driver.
	\item \verb"lib": gestisce le librerie di Arduino, consente di installare nuove librerie e di visualizzare la lista di quelle installate
	\item \verb"board": visualizza i tipi di board e su quali porte sono connesse e la lista di tutte le board compatibili con l'interfaccia
\end{itemize} 

\subsection{ESP8266WebServer}

ESP8266WebServer è la libreria compatibile con la tipologia di board NodeMCU, impiegata per implementare il Web Server. 

Per definire un Web Server, innanzitutto, è necessario stabilire il numero della porta sulla quale esporlo ed i vari endpoint sui quali rimanere in attesa di richieste da parte dei client. Un endpoint viene definito tramite il metodo\\\verb"on(endpoint, tipo_di_richiesta, handler)", il quale prende in input tre parametri: l'URL della richiesta su cui rimanere in attesa, il tipo di richiesta, ad esempio \verb"HTTP_GET" o \verb"HTTP_POST", e l'handler. L'handler è una funzione che viene invocata per gestire la richiesta e al suo interno deve essere presente il messaggio di risposta, che viene inviato al client tramite il metodo\\ \verb"send(codice_http, tipo_risposta, risposta)". Il primo parametro costituisce il codice HTTP della risposta, ad esempio \verb"200" per indicare il sucesso o \verb"400" per indicare un errore, il secondo rappresenta il tipo della risposta (content-type) che può essere \verb"application/json" o \verb"text/plain" ad esempio e infine, il terzo parametro contiene il messaggio di risposta che viene inviato al client.

Definiti questi parametri, è possibile far avviare il server e connetterlo ad Internet in modo da poter gestire tutte le richieste definite precedentemente. Per avviare un server si utilizza il metodo \verb"begin()", mentre per gestire le richieste si utilizza il metodo \verb"handleClient()".

La libreria ESP8266WebServer si base sul protocollo HTTP al livello applicazione e sul protocollo TCP/IP al livello trasporto.
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=3pt]
	\item \verb"WStype_CONNECTED": questo messaggio viene inviato dal client la prima volta che si connette al server WebSocket. Nel momento in cui verrà ricevuto dal server, esso aprirà il canale WebSocket e lo terrà aperto finché il client si disconnette o cade la connessione.
	\item \verb"WStype_DISCONNECTED": questo messaggio viene ricevuto dal server nel caso in cui un client, con cui ha aperto un canale di comunicazione, si dovesse disconnettere.
	\item \verb"WStype_TEXT": questo tipo è il più diffuso, infatti identifica tutti quei messaggi che vengono scambiati tra client e server durante le loro interazioni.
	\item \verb"WStype_PING" e \verb"WStype_PONG" corrispondono rispettivamente ad un messaggio di PING e di PONG. Solitamente un messaggio di PING viene inviato automaticamente dal mittente prima di un qualsiasi scambio di messaggi con il destinatario oppure viene inviato dal client ogni tot intervalli di tempo per mantenere il canale attivo.
\end{itemize}	
	
Per avviare un server WebSocket	si utilizza il metodo \verb"begin()", come avviene per il server Web e successivamente si definisce la funzione che intercetta e processa gli eventi attraverso il comando \verb"onEvent(funzione_gestione_eventi)" e lo si fa mettere in ascolto di eventi tramite il metodo \verb"loop()". La funzione di gestione degli eventi viene chiamata ogniqualvolta si riceve un messaggio e al suo interno è definito un costrutto \verb"switch-case", il quale intercetta il tipo di messaggio, si sposta nel ramo \verb"case" corrispondente e infine lo processa. Per memorizzare l'elenco dei client connessi, il server mantiene una tabella in cui associa all'IP di ogni client un id numerico. Questo id è progressivo e viene incrementato mano a mano che si aggiungono client.

Il server può rispondere al client inviandogli un messaggio attraverso il metodo \verb"sendTXT(num, messaggio)", dove \verb"num" è l'id del client. Il messaggio può essere solo di tipo stringa.

\subsection{ArduinoJson}

ArduinoJson è la libreria che consente ai sistemi embedded di parsare dati in formato JSON. Ho dovuto utilizzare questa libreria poiché la Thing Description e tutti i messaggi scambiati tra Thing e Consumers seguono questo formato. 
...
\subsection{Thing CLI}

La Thing CLI mette a disposizione dell'utente quattro comandi, ognuno dei quali implementa una funzione specifica: \verb"start", \verb"build", \verb"compile" e \verb"flash".

\vspace{0.5cm}

\begin{table}[h]                       %crea l'ambiente figura; [h] sta
	%   per here, cioè la figura va qui
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=13cm]{TC.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Interfaccia Thing CLI]{Interfaccia Thing CLI}
	\end{center}
\end{table} 

Attraverso il comando \verb"start", l'utente può avviare la procedura guidata di definizione della TD e di tutte le informazioni necessarie al file di template per generare il codice del file di scripting. Come prima informazione l'utente dovrà inserire il nome della Thing, il quale poi verrà visualizzato nell'URL della home del server web. Successivamente dovrà inserire sia i meta-dati globali della Thing, sia quelli specifici di ogni Interaction Affordance. I meta-dati più importanti sono quelli contenuti nell'array \verb"forms", in cui vanno inseriti: l'endpoint dell'interaction, il tipo di contenuto della richiesta (per tutte le richieste è \verb"application/json") e il tipo di operazioni supportate per quella richiesta. Il tipo di operazioni supportare variano da risorsa a risorsa, ad esempio per una proprietà è possibile leggere e scrivere il suo contenuto specificando, rispettivamente, il meta-dato \verb"readproperty" e \verb"writeproperty", per un'azione è possibile invocarla con \verb"invokeaction", mentre per un evento è possibile sottoscriversi con \verb"subscribeevent" e cancellarsi con \verb"unsubscribeevent". 

Per quanto riguarda le proprietà, è necessario specificarne il tipo. I tipi supportati sono: \verb"boolean", \verb"integer", \verb"number", \verb"string", \verb"null", \verb"array" e \verb"object". Lo stesso vale anche per i parametri in input e l'output delle funzioni invocate dalle azioni e per i campi degli schemi degli eventi (sottoscrizione, dati e cancellazione). Ogni tipo poi ha associati dei sotto-campi specifici, ad esempio il valore massimo e minimo di un array, le proprietà di un oggetto, il tipo degli elementi di un array ecc. A questo scopo è stata implementata una funzione che prende in input il tipo base e dinamicamente permette all'utente di inserire i relativi sotto-campi.

Sono stati implementati dei tipi di dato personalizzati per far fronte ai tipi specifici del formato JSON, come ad esempio stringhe il cui primo carattere non sia un numero, interi non negativi, interi diversi da zero, oggetti JSON e così via. Per definire un tipo personalizzato utilizzando la libreria Click, è necessario ridefinire il metodo \verb"convert()" della classe \verb"click.ParamType", il quale viene invocato dalla libreria per verificare se un dato ha o meno quel tipo specificato, se non lo ha si restituisce un errore. In questo caso l'utente dovrà ri-inserire un nuovo dato del tipo corretto.
...
\begin{table}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=14cm, height=4cm]{TP.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Esempio di definizione di un tipo di dato personalizzato per la libreria Click]{Esempio di definizione di un tipo di dato personalizzato per la libreria Click}
	\end{center}
\end{table} 
...
Terminato l'inserimento di tutti i meta-dati delle TD si chiede all'utente se vuole generare il file di scripting o terminare l'interazione con l'interfaccia. In entrambi in casi, comunque, viene generato un file JSON contenente la TD appena definita. I meta-dati sono memorizzati in un dictionary di Python incluso nel contesto (\verb"ctx") dell'interfaccia Click e dato che il contesto non si perde tra un comando e l'altro ma viene passato ad ognuno di loro, i dati memorizzati al suo interno possono essere acceduti in qualunque momento.

Nel caso in cui l'utente continui l'interazione con la CLI, gli verrà chiesto di inserire le informazioni necessarie al template engine per generare il file di scripting. Se l'utente, invece, avesse già a disposizione una TD memorizzata in un file JSON, potrebbe saltare la procedura precedente e attraverso il comando \verb"build" darlo in input alla CLI, la quale estrarrà i dati e li memorizzerà nel dictionary corrispondente.

Le ultime due operazione che l'utente può svolgere sono la compilazione e il flashing del file di scripting sulla board NodeMCU tramite i comandi \verb"compile" e \verb"flash". Per esporre queste due funzionalità viene sfruttata l'interfaccia arduino-cli, i cui comandi vengono eseguiti in background in modo trasparente per l'utente. La procedura di interazione con arduino-cli si articola nel modo seguente:

\begin{enumerate}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	\item In primo luogo si verifica se l'interfaccia arduino-cli è disponibile ed in caso contrario si procede all'installazione dell'ultima versione rilasciata.
	\item In secondo luogo si controlla se sono presenti i driver per il NodeMCU ed in caso negativo si prosegue come sopra.
	\item Come terzo punto si chiede all'utente di installare le librerie aggiuntive, rispetto a quelle di default (ESP8266WebServer, WebSockets e ArduinoJson), che ha utilizzato nel file di scripting.
	\item Infine si procede alla compilazione e/o al flashing del codice. Importante: l'operazione di flashing necessita che la board sia connessa tramite una porta seriale.
\end{enumerate}
...
Il file di template segue la struttura degli sketch scritti attraverso l'IDE di Arduino, quindi all'inizio si includono le librerie e si definiscono le variabili globali, poi si implementano i due metodi principali di un programma per Arduino, \verb"setup()" e \verb"loop()" e infine si definiscono le funzioni. 
...
Terminata questa fase, si procede all'inclusione, prima delle librerie di default, poi di quelle aggiunte dall'utente, le quali, essendo memorizzate in un array, vengono inserite una per una tramite un ciclo \verb"for". Successivamente si definiscono le variabili relative al ssid e alla password della rete Wi-Fi a cui il server si dovrà connettere, ai tipi di protocolli utilizzati (http e ws), ai numeri delle porte su cui esporre il server web e websocket, al nome attribuito alla Thing e i documenti JSON per gestire le proprietà di tipo complesso (oggetti o array) e gli eventi WebSocket.

Le proprietà, le azioni e gli eventi, come avviene per le librerie, sono memorizzati all'interno di array così da avere la possibilità di iterare tra i loro elementi tramite cicli \verb"for". Per ogni proprietà si definisce il nome, il tipo e il valore iniziale. Per le azioni si definisce il nome, il numero di input e lo schema per ognuno di essi. Per gli eventi, infine, si dichiara il nome ed i vari schemi di sottoscrizione, notifica e cancellazione. 
...
Con l'inserimento delle richieste e degli oggetti server termina la definizione delle variabili e si passa alla compilazione dei metodi \verb"setup()" e \verb"loop()".

Il codice del \verb"setup()" viene eseguito solamente una volta dal microcontrollore e come prima istruzione. Nel \verb"setup()" si connette il server ad Internet, si dichiara la variabile di tipo stringa a cui viene assegnata la TD, la quale verrà esposta sulla home del server, si configura il server per la gestione degli endpoint tramite il metodo \verb"on()" e alla fine si avviano i due server.

Il codice del \verb"loop()" invece viene eseguito di continuo dalla board e contiene, di default, solamente due istruzioni: la chiamata al metodo \verb"handleClient()", in modo tale che il server web possa gestire tutte le richieste in arrivo dai client e la chiamata al metodo \verb"loop()", affinché anche il server WebSocket possa fare altrettanto.

All'interno dei metodi \verb"setup()" e \verb"loop()", l'utente può inserire istruzioni aggiuntive tramite la Thing CLI.

Terminata questa fase, si passa alla dichiarazione degli handler per gestire le richieste e delle funzioni associate alla azioni. Possono essere presenti ulteriori funzioni aggiunte dall'utente tramite la Thing CLI a completamento o a supporto delle funzionalità della Thing. Tra le funzioni, poi, ne è presente una, \verb"handleInputType()", che ha il compito di verificare la correttezza del contenuto delle richieste POST per invocare le azioni. Controlla se, per ogni parametro da passare in input all'azione, il valore corrispondente presente nel messaggio di richiesta del client ha il tipo corretto. Se la verifica non va a buon fine, l'azione non viene invoacata. Gli input delle azioni vengono inviati dal client all'interno del corpo della richiesta e non come parametri nell'URL.
...
In conclusione, per le Interaction Affordances è stato supportato sia il protocollo HTTP, sia il protocollo WebSocket, tranne che per gli eventi, nel confronto dei quali non è stato possibile implementare il protocollo HTTP di long polling poiché le tecnologie a disposizione non lo hanno permesso.
...
\section{Caso d'uso: Smart Parking}
...
\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	\item aiutare i guidatori a trovare parcheggio facilmente e velocemente
	(es. tramite mobile apps)
	\item mettere a disposizione informazioni che aiutano i vigili o altri incaricati a identificare le eventuali violazioni
	\item aiutare le persone a scegliere mezzi di trasporto alternativi nel caso in cui i parcheggi siano esauriti
	\item ridurre la congestione del traffico
	\item migliorare le condizioni ambientali: emissione di gas tossici e
	qualità dell'aria
\end{itemize}
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=13cm]{SP.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Smart Parking]{Smart Parking}
	\end{center}
\end{figure} 
...
L'obiettivo dei sensori è quello di verificare che un'area di parcheggio sia disponibile oppure no e comunicarlo al sistema di parcheggio tramite un
gateway, in modo tale che il guidatore ne sia notificato. Esistono diverse tipologie di sensori, tra i quali, i più importanti sono: le videocamere, gli smartphone, i sensori ultrasonici, ad infrarossi, radar, magnetometri e geomagnetici.

I protocolli di rete rappresentano lo strumento per trasferire le informazioni raccolte dai sensori al sistema di parcheggio. Nello smart parking esistono due tipologie di protocolli di rete, una è rivolta
agli utenti e l'altra viene utilizzata dai sensori (in alcuni casi se ne ha soltanto una utilizzata da entrambi).

L'architettura di rete deve essere in grado di supportare i centinaia di
migliaia di utenti che potenzialmente possono accedere alla rete e deve
rispettare le seguenti caratteristiche: basso consumo di energia, minimo
ritardo e affidabilità nella trasmissione dei dati. I principali protocolli di rete sono: protocolli wireless per IoT (LPWAN e LR-WPAN come LoRa, NB-IoT e ZigBee), il WiFi, la tecnologia Bluetooth, i protocolli cellulari (3G/4G) e cablati (Ethernet, USB e comunicazione seriale).

La componente rappresentata dai sistemi software è di vitale importanza poiché consente di manipolare le informazioni ricevute dai sensori. Le soluzioni software non devono svolgere soltanto il compito di
immagazzinamento dei dati ma deve soprattutto fornire dei servizi che
possano aiutare gli utenti a risparmiare carburante, tempo e denaro.

Le principali funzioni svolte da un sistema software devono essere:

\begin{itemize}[itemsep=0pt,partopsep=1pt,parsep=5pt]
	\item verifica della disponibilità di un parcheggio (libero, occupato, prenotato)
	\item prenotazione di un parcheggio
	\item pagamento della sosta
	\item comunicazione eventuali violazioni
	\item prolungamento della sosta
\end{itemize}
...
Il modo migliore per immagazzinare e analizzare l'enorme quantità di
informazioni raccolte dal sistema software è tramite le piattaforme cloud.
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=14cm]{SENSORS.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[I tre sensori, da sinistra verso destra: l'HC-SR04, l'Adafruit VL53L0X e lo Sharp IR]{I tre sensori, da sinistra verso destra: l'HC-SR04, l'Adafruit VL53L0X e lo Sharp IR}
	\end{center}
\end{figure} 
...
Ogni sensore gestisce un parcheggio e verifica se è libero oppure se è occupato da un'automobile. Lo stato di un parcheggio viene visualizzato sia in un display LCD posizionato all'entrata del parcheggio (A -$>$ disponibile, O -$>$ occupato), sia nella Thing Description esposta dalla Thing. La logica della Thing è stata implementata tramite il sistema Embedded WoT Servient oggetto di questo lavoro di tesi.

Nella TD sono state definite quattro proprietà di tipo oggetto, la prima corrisponde al numero di slot disponibili, mentre le restanti tre identificano i tre parcheggi. Ognuna di queste tre proprietà ha due campi: lo stato del parcheggio, un booleano che è \verb"true" se il parcheggio è disponibile ed è \verb"false" nel caso opposto e la soglia del sensore identificata da un numero intero. Ogni sensore ha un diverso range di misurazione, per cui le soglie hanno dei valori massimi e minimi che vanno rispettati. Oltre a queste proprietà sono state aggiunte tre azioni, le quali permettono all'utente la modifica delle soglie dei vari parcheggi. 
...
\begin{table}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=5cm]{TDSP.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Elenco delle proprietà definite nella TD esposta dalla Thing]{Elenco delle proprietà definite nella TD esposta dalla Thing}
	\end{center}
\end{table} 
...
\begin{enumerate}
	\item Nel primo test si è impostato un valore di soglia pari a 6 per tutti e tre i sensori e sono state condotte 10 misurazioni per ogni parcheggio. Per ogni simulazione si è verificato il corretto funzionamento del sensore.
	\item Nel secondo test si sono variate le soglie dei sensori e si sono eseguite 10 simulazioni su ogni parcheggio, per ogni diverso valore della soglia. I valori di soglia scelti sono stati: 5, 10, 20.
\end{enumerate}
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=12cm]{T1.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Test 1]{Validazione-Test 1}
	\end{center}
\end{figure} 
...
\begin{figure}[h]                       %crea l'ambiente figura; [h] sta
	...
	\begin{center}                          %centra nel mezzo della pagina
		%   la figura		
		\includegraphics[width=12cm]{T2.png}%inserisce una figura larga Xcm
		%se si vuole usare va scommentata
		%
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%inserisce la legenda ed etichetta
		%   la figura con \label{fig:prima}
		\caption[Test 2]{Validazione-Test 2}
	\end{center}
\end{figure} 
...
L'obiettivo di tale sperimentazione non è quello di confrontare tra loro sensori diversi, ma dimostrare la possibilità di gestire sensori eterogenei attraverso la piattaforma Embedded WoT Servient. 
...
In questo elaborato è stata presentata la realizzazione della piattaforma Embedded WoT Servient per l'esposizione di Thing, basate sullo standard del W3C, su sistemi embedded. 
...
Il lavoro svolto si è rivelato molto lungo e impegnativo, data la complessità del progetto, il cui scopo era far approdare l'articolata architettura del WoT Servient nel mondo dei sistemi embedded. 
La principale difficoltà riscontrata è legata al trasferimento delle funzionalità avanzate dell'architettura del WoT Servient ai sistemi embedded, i quali possiedono risorse limitate e ridotte capacità computazionali. Inoltre, si voleva proporre un sistema altamente intuitivo e facilmente fruibile, con il quale l'utente potesse interagire con il minor numero di passaggi possibile.  

Per quanto riguarda gli sviluppi futuri, l'intenzione è quella di passare da un'interfaccia a riga di comando ad un'interfaccia grafica, sicuramente più intuitiva e accessibile oltre che di identificare altri casi d'uso.

Il lavoro non deve essere considerato definitivamente concluso, infatti sarà necessario mantenerlo costantemente allineato con l'evoluzione dei documenti e degli standard del W3C. Non sono rare, infatti, modifiche sostanziali all'architettura tra una versione e l'altra degli standard. 
...
\bibitem{K1} Satista. \textit{Internet of Things (IoT) connected devices installed base worldwide from 2015 to 2025} (2016). [ultima visita 07.03.2020]. URL: \url{https://www.statista.com/statistics/471264/iot-number-of-connected-devices-worldwide/}.
\bibitem{K2} Shancang Li, Li Da Xu, Shanshan Zhao, \textit{5G Internet of Things: A Survey}. In: Journal of Industrial Information Integration,
Volume 10. Pagine 1-9. ISSN 2452-414X (2018). 
\bibitem{K3} Shancang Li, Li Da Xu, Shanshan Zhao, \textit{The internet of things: a survey}. In: Information Systems Frontiers. Pagine. 243-256 (2015).
...
Con questo mio ultimo lavoro si conclude un percorso di studi fatto di tanti momenti felici ma anche tanti momenti in cui tenere duro era veramente difficile. Se mi guardo indietro ricordo tutta la fatica per preparare esami davvero complessi ma anche la travolgente gioia ogni volta che tutti i miei sforzi venivano ripagati. Ho conosciuto molte persone che mi hanno accompagnato in tutto il mio periodo di studi condividendo insieme a me lati positivi e lati negativi dell'essere degli instancabili programmatori. Tra questi sicuramente un posto speciale nel cuore ce l'ha Giuseppe. Senza di te Perugia non sarebbe stata la stessa. Grazie per essere stato non solo un compagno di studio e di divertimenti ma sopratutto un leale e sincero amico per la vita. 

Innanzitutto vorrei ringraziare la mia famiglia che ha sempre creduto in me e, anche nei momenti più difficili, mi ha sempre incoraggiato a non mollare mai nonostante le apparentemente insormontabili difficoltà. In particolare ringrazio mio padre Nazzareno senza il cui sostegno, probabilmente oggi non sarei qui. Grazie papà e grazie mamma perché avete creduto in me dandomi la forza necessaria per andare avanti anche con qualche rimprovero, forse necessario. Ho cercato di seguire il vostro esempio dando sempre il massimo come fate voi ogni giorno prendendovi cura di nonno Pietro e nonna Claudia. Nonna Rosa sarebbe molto fiera di voi.

Nonna Rosa, hai visto dove sono arrivato? Mi hai sempre detto ``Studia che poi te lo ritrovi''...avevi proprio ragione! I tuoi insegnamenti sono stati per me fondamentali e hanno contribuito a formare il mio carattere. Grazie perché se sono diventato il ragazzo che sono ora lo devo anche a te. So che mi stai guardando in questo momento, spero tu sia fiera di me e di quello che sono diventato.

Ringrazio te Vanessa, per avermi supportato e sopportato durante tutto il mio percorso universitario. Sei sempre stata dalla mia parte, hai sempre creduto nelle mie possibilità e mi hai trasmesso tanti piccoli insegnamenti di vita. Sono cresciuto molto da quando sei nella mia vita non solo anagraficamente ma anche emotivamente e personalmente. Grazie alle esperienze di vita vissuta insieme, attualmente non potrei pensare alla mio futuro senza di te perché sei diventata una parte integrante di me senza la quale il mio codice crasherebbe. Senza di te Momi, questi sette anni non sarebbero stati gli stessi!

Una menzione speciale la riservo per Bologna. Bologna ha avuto per me sempre una preferenza speciale. Fin dagli inizi, ho sempre avuto la sensazione che fosse fatta per me, che ci fosse stato spazio anche per me...e avevo ragione. Bologna mi ha saputo accogliere senza farmi sentire inadeguato, fuori posto o diverso. 
 
Per concludere, vorrei dedicare due righe a me stesso, un ragazzo spesso molto timido che ha saputo farsi strada in questa nuova avventura bolognese. Non me lo dico molto spesso ma sono fiero del ragazzo che sono ora e di dove sono arrivato. Perseveranza e forza di volontà nella vita ripagano sempre! 
